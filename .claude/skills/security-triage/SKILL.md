...
Read `SECURITY-NOTES.md` and `.zap/rules.tsv` first — they are the source of truth for what is
already accepted.
...
3. **Diff against accepted risks.** Compare the plugin ids against the IGNORE list in
   `.zap/rules.tsv` (each id is cross-referenced to a numbered accepted risk in
...
4. **Triage each NEW finding — one verdict, no third option:**
   ...
   - **Accept** → add its plugin id to `.zap/rules.tsv` as IGNORE **and** a matching numbered
...
- The scan is a **tripwire**: `fail_action: true` means any id NOT in `.zap/rules.tsv` turns the
