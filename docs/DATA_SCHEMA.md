...
| `rest:YYYY-MM-DD` | v179 | `"1"` or `""` |
| `garden` | v183 | see below |
...
A per-day rest flag. Combined with a check-in it produces a **full** bloom, not a reduced one — a day
of rest is a complete day, and the flower is never a score.

### `garden`

```js
{ "YYYY-MM-DD": { bloom: 0..1, intention?: string, rest: bool, mood?: 1..5 } }
```

One saved flower per day. Written by `saveGardenToday()` on every repaint, but **only for a day that
had presence** — a check-in, a ritual done, or a rest day. Otherwise the key is removed rather than
stored as a zero.

The continuous write *is* the snapshot: past days freeze on their own, so there is no midnight job to
get wrong. Included in backup. Rendered by the Garden overlay via `buildMiniFlower`; the monthly
reflection reads it together with `rituals.log` and `settings.identity`.

> Introduced on the `staging` branch with the Living Flower arc. Check which build is promoted before
> assuming it is in production.
