...
function renderDerived(phases, branch) {
  const lines = [
    START,
    '',
    ...
    // It is the one field in this block that does not come from repository content, which is why
    // --check takes it from the file rather than from the checkout. See the note at the call site.
    //
...
// The branch line is the one part of the block a check cannot verify everywhere, so it is not
// verified. On a pull request GitHub checks the code out detached at the merge ref, where
// `rev-parse --abbrev-ref HEAD` answers "HEAD" rather than a branch name, so a check that insists on
// the committed value fails on every pull request by construction. It did exactly that, twelve
// seconds into the first run this branch ever had in CI, which is also the first time this check had
// ever run anywhere but a laptop.
//
// So --check compares everything the repository can produce and takes the branch from the file it is
// checking. The line still records which worktree the evidence was read from, which is the only
// reason it exists, and it is written by the run that regenerates the block, on a machine where the
// branch is a real branch. Reproduced before it was changed, with `git worktree add --detach`.
const checking = process.argv.includes('--check');
const committedBranch = (current.match(/^- Worktree branch: `(.+?)`\s*$/m) ?? [])[1];
const branch = (checking && committedBranch) || git(['rev-parse', '--abbrev-ref', 'HEAD']);
const rebuilt = current.slice(0, startAt) + renderDerived(phases, branch) + current.slice(endAt + END.length);
