> **APP-HERITAGE.md (D-44, 7 iul 2026).** Acesta e fostul CLAUDE.md complet — spec-ul TEHNIC autoritar al aplicatiei-zestre (mi-dia-vNN.html, prod v172 / staging v184 / v185 WIP). CITESTE-L INTEGRAL inainte de ORICE lucru pe app, index.html, sw.js, e2e sau skill-urile /ship /staging /design-check /theme-qa /revamp. Orice referinta veche la "CLAUDE.md" ca spec tehnic inseamna acum ACEST fisier. CLAUDE.md a devenit slim (faza + ritual de start + pointere).

# Mi Día — Project Context for Claude (canonical filename: CLAUDE.md)

> **Authoritative TECHNICAL spec. Read this first, every session, before any work.**
> Last updated: 7 July 2026 · **PROD (`main`, live on Cloudflare) = `mi-dia-v172.html`** (`index.html`, sw CACHE `mi-dia-v172`).
> **WIP arc on branch `staging` = `mi-dia-v184.html`** (preview `staging.mi-dia-app.pages.dev`, NOT promoted to prod).
>
> ⭐⭐ **CURRENT PHASE (July 2026) — COMPANY CREATION, not app work. READ `private/mi-dia-status.md`
> FIRST** — it is the live status/onboarding map (what's done, what's next, the working style).
> In one line: the conceptual foundation (Constitution v1.8 · Theory of Human Reconnection v2.0 ·
> Philosophy ~80% · the 7-day MVP Experience sprint, all canonical, D-13…D-42 in
> `private/design-decisions.md`) is FINISHED and FROZEN; work pivoted to **product cycles in a
> live laboratory**: a SEPARATE Cloudflare project **`mi-dia-lab.pages.dev`** (source
> `private/prototype/site/`, own local git, deployed via wrangler direct upload — NEVER through
> this public repo). **Cycle 01 (Arrival) is live at v4**, iterated with Ines as participant
> zero (Experiment 001). **THIS app (`mi-dia-vNN.html`) is untouched heritage** — zero code
> changes in this phase; its divergences from the new foundation are MARKED in
> `private/reconciliation-register.md` (R-01…R-15) and remediated only by explicit per-entry
> decisions, later. The v185 watercolor-flower WIP below remains parked.
>
> 🌸 **NEWEST WIP — v185 flower REDESIGN → WATERCOLOR (LOCAL, uncommitted, on `staging` working tree, NOT on staging-live/prod).**
> Ines revived the flower redesign (D-10 was "abandoned") with **real watercolor bougainvillea bloom-series
> assets**. The geometric SVG flower (petalPath, 5 petals) is replaced on **Home + the Garden mini-flowers** by
> an **illustration that blooms** via a **crossfade that SNAPS to the nearest frame** (a continuous blend of two
> different illustrations reads as a ghosted/blurry double at rest → snap keeps every resting state ONE crisp
> frame; the frame-to-frame transition still crossfades). `renderFlower(bloom,bonus)` mechanics KEPT (PRESENCE
> recipe, `role=progressbar`, reduced-motion, reveal on check-in, micro-scale + gilt halo + breathe); only petal
> rendering → a stack of `<img.wcf>` from `const WCFRAMES`; `buildMiniFlower` → the frame nearest the day's bloom.
> Centre pink disc hidden (the watercolor has its own painted heart; intention stays in the pill below). Same
> asset both themes + a dark saturation lift. Flower made **smaller** (340→266px) + diffuse restrained shadow.
> **Source** (in `bougFlowers/`, uncommitted): hi-res individual `bloom1..bloom6` + `bloom9` (1536px, transparent)
> → **7 stages** extracted via canvas (bbox+centre+growth ramp+alpha-clean+**unsharp**+WebP q0.90), natural bud→full.
> Validated (div 241/241, node --check 3/3). File ~1.09MB (from 678KB). **LEFT:** `bloom7`+`bloom8` (the middle,
> Ines tomorrow) → extend to ~9 stages (kills the last jump); colour/gold enrichment (ChatGPT prompt given); prune
> dead CSS (`.petal`/`.gpetal`/`#petalPath`) as a separate felie; sync onboarding `flowerIllus` (still geometric
> line-art); re-run e2e on the final set; then `/staging` (device pass) + eventual `/ship`. Preview: local http-server `:8185`.
> See memory `living-flower-build-progress` + `CHANGELOG.md` (v185) + `design-asset-workflow`.
>
...
Personal use for now (localStorage only). Future: public version — **lifetime pay-what-fair,
never a subscription** (constitutional non-negotiable, D-13).

**Owner:** Ines — QA/AI professional based in Spain. Comfortable with Node + GitHub.

**⭐ Vision law (D-13, July 2026): the Mi Día Foundation Constitution** —
`private/marketing/mi-dia-constitution.md` (v1.4 — adopted D-13; amended D-14/D-15, **D-17: Mi
Día does not build habits, it builds a RELATIONSHIP; rituals are the language — "habits are only
the seeds"**, and **D-18: THE SANCTUARY PRINCIPLE ("a place to safely return to themselves, not
to fix themselves"), THE HUMAN FIRST PRINCIPLE (no AI-led guidance — "We help you discover it
yourself"), and the REAL CATEGORY = "self-connection companion"**) is the SUPREME
vision/ethics document. Order of authority: **Constitution → its operating laws (Research OS
`private/research-os.md`, method — draft; **the Human Change Model**
`private/mi-dia-human-change-model.md` — **the Theory of Human Reconnection, v2.0 CANONICAL
DRAFT** (D-20…D-23; renamed from "theory of change": change = consequence, reconnection = the
central phenomenon), a declared **PRODUCT THEORY** (coherent·useful·testable·true-enough — not
scientific truth), NOT YET PUBLISHED; its core: the Human Premise ("adaptive, not broken" —
never start from repair), the Fundamental Law (authorship: "we become the authors of our lives
by transforming our relationship with our experiences"), **"Safety is not comfort. Safety is
permission."**, the Law of Attention (transversal: "attention is participation in reality"),
the **SEVEN-capacity cycle Safety·Observe·Understand·Honor·Express·Integrate·Belong**, identity
("we become who we repeatedly experience ourselves being"), the Product Principle (a feature is
valuable ONLY if it strengthens a capacity), human success metrics ("I feel at home in my own
life"), **the Mi Día Axiom** ("Every lasting transformation begins as a transformation in the
relationship a person has with their own experience. Our role is not to create that
relationship, but to create the conditions in which it can naturally grow."), + Annex A: the
Human Experience Framework (6 dimensions; healthy = 4 verbs "fără a te pierde"; the
deterioration cascade) and Annex C: the adoption gate (per-capacity observable definitions,
self-repair limit, Phase-1 data, harmonizations BOS §35 + 6-dim/7-doors); **⛔ STRATEGIC FREEZE: no Experience Principles 2.0 / UX
Architecture 2.0 / Design System / Growth / Marketing derivatives are written until HCM v1.0** —
lower documents must become inevitable consequences, not invented rules; its crystallized causal
core = **the Theory of Change**
`private/mi-dia-theory-of-change.md` — D-19, a falsifiable WORKING HYPOTHESIS:
"change comes not when a person sees herself, but when she believes herself" — safety → attention
→ … → identity → freedom; connection = the MECHANISM, an authentic lived life = the purpose;
predictions P1–P6 to be tested in roadmap Phase 1; BOS Vol. I `private/marketing/mi-dia-bos-vol1.md`,
product experience — **v1.5, 41 sections, complete**: Purpose→Product Manifesto + the §5
transformation ladder + the inner-growth chapters §35–40 (Three Waters: Awareness/Care/Alignment;
Listening; Cultivation; Flower as Witness), D-15/D-16/D-18) →
brand bible/brief (`private/marketing/`) → design-decisions journal
(`private/design-decisions.md`) → everything else, including the already-built app** (this file
stays the TECHNICAL spec). The BOS ends in a 4-level question cascade every piece of work answers:
Constitution's 12-question filter (company) → Golden Question §33 (product: "does this strengthen
the relationship a person has with themselves?") → Experience Test §23 (feature: self-connection
vs mere engagement) → Design Question §34 (screen: "does this help the user return to themselves?").
Any new feature, copy, campaign or business decision must pass its 12-question decision filter.
Hard constitutional lines every agent must know: no shame/guilt/hurry ever (no "0/N", no red
zero — D-08); rest day = full bloom (D-07); the flower is never a score; measurement = gentle
proxies of presence, always interpreted in the user's favor; **privacy is a precondition** (data
on device, no account, no surveillance; any future AI acts only at the user's request, never
diagnoses); **truthfulness by restraint** (evidence hierarchy — contested theories are not used:
the "vagus nerve" removal is precedent); companion-not-clinician; monetization = core free
forever, one-time fair payment, never a subscription, never paywalling her own reflections.
The public one-page manifesto (trilingual) lives in `private/marketing/mi-dia-manifesto.md`.

**⭐ The Relational Nucleus — FROZEN (D-25, July 2026):** Constitution v1.6 — *"Mi Día este o
companie dedicată cultivării relațiilor care fac posibilă înflorirea umană."* The quality of a
life is limited by the quality of the relationships with one's own experience, with others, and
with the world; Mi Día develops the QUALITY OF RELATIONSHIPS (meta-capacity: Conscious
Relating); a relationship = "un mod repetat de a întâlni ceva"; attention = "actul prin care
devenim disponibili pentru întâlnire". Every feature asks "what relationship does this
strengthen?", never "what behavior does this change?". **New ideas never reshape this nucleus —
they are evaluated against it.** Method (D-25): documents are now written CANONICALLY, one by
one, foundation untouched between them ("opera, nu conversație"). **The translation rule (D-26,
Constitution v1.7): a lower-level document may NEVER introduce a new fundamental idea — each
level only TRANSLATES the level above; new fundamentals enter only at the top, by explicit
amendment.** Strict canonical order (D-26): Philosophy v1.0 (ch. I incl. "what is a PERSON",
V Health, VII Flourishing, editorial pass) → Theory re-derived from Philosophy → Grammar →
Experience Architecture (fully traceable). "No more «cred că» — only derivation." **D-27:**
Grammar CONFIRMED as its own level (rules ≠ structure; levels renumbered L0–L8: Grammar=L2,
Architecture=L3, …Growth=L8); Philosophy ch. I method = OPERATIONAL PREMISES about the person,
never definitions ("Mi Día pornește de la următoarele premise despre persoană:"; first three
fixed; guard: no loaded values — the person characterized by the capacity to relate); the goal =
a DERIVATION SYSTEM ("explainable, not just consistent"). Process rule (memory
`flag-dont-silently-fix`): inconsistencies are RAISED with the existing law kept, never silently
repaired. **D-28 (Constitution v1.8):** THE RULE OF OPERATIONAL PREMISES — where ontological
definitions are impossible, Mi Día formulates operational premises + limits of reduction, never
absolute definitions; Philosophy ch. I "The Person" is CANONICAL v0.8 (6 premises P1–P6, Ines's
text verbatim). **D-29:** P0 relocated, not rejected — it failed the independence + dignity
tests as ontology and became **the PHENOMENOLOGICAL AXIOM** at the head of the Philosophy
("oamenii nu trăiesc viața direct — o trăiesc prin relațiile pe care le construiesc…"; how life
is LIVED, never what the person IS); two categories now distinct: the axiom (before ch. I) vs
the central person-premise (ch. I, still open — new proposal "a person precedes and exceeds her
relationships" recorded, awaiting Ines). **⭐ D-30 PIVOT (amends D-20/D-26, Ines's explicit
call): Philosophy paused at ~80% — the next objective is the MVP EXPERIENCE
(`private/mi-dia-mvp-experience.md`): the 3-minute first experience, 7-day sprint (Opening ·
Today's Moment · Reflection · Emotion · Ritual · Closing · Tomorrow), no UI/Figma — experience
only; every UX decision = a TEST of the philosophy (which law does it translate? which
prediction P1–P6 does it test?); then the landing page (sells the experience, never the theory)
→ feedback → Theory v2. The freeze on canonical LAW documents stays; the prototype is an
experiment, not a law. Still zero app code; the register R-01…R-15 untouched.**

**⭐ The Document System (D-24, July 2026):** the foundation corpus is a TREE of abstraction
levels, mapped in **`private/mi-dia-document-system.md`** — L0 Philosophy
(`private/mi-dia-philosophy.md`, v0.1 — canonical writing in progress: first sentence + ch.
II/III/IV/VI done; ch. I/V/VII to write, V=health is the open question) → L1 Human Change Model (exists, v2.0) → **L2a Human Experience Grammar** (missing; the
FIRST derivative after the adoption gate — "Experience Principles 2.0/UX Architecture 3.0" is
retired, never to be written) → L2b Experience Architecture → L3 Product → L4 Interaction → L5
Visual → L6 Voice → L7 Growth. Constitution + Research OS + decision journal + reconciliation
register are TRANSVERSAL (govern all levels). Terminology: L0-L1 = "operational model"
(hypotheses under permanent test), never presented as scientific truth. BOS v1.5 mixes levels
2–6 and will be refactored BY LEVEL at the cascade (absorbed with supersession stamps, never
deleted). A lower-level document derives from above, constrains below, contradicts neither.

**⭐ Principles-first rule (D-14/D-15, July 2026) — every agent must follow this:** the foundation
stack is Constitution → **BOS Vol. I** (`private/marketing/mi-dia-bos-vol1.md` — the law of
how the product should FEEL, the canonical merge of Ines's drafts) → brand bible/brief. **The
rules are NEVER edited to fit the already-built app** — the app is measured against the rules.
Gaps (**R-01…R-14**: streak chip, ritual series, "N/N done" on Home, Progress metrics, "miss"
state, palette-vs-BOS, navigation, planner, focus timer, memento, check celebration, execution
copy, no arrival transition, quiet-luxury vs unattainable-lifestyle) are MARKED, one entry each,
in **`private/reconciliation-register.md`** (+ its 1:1 HTML mirror `reconciliation-register.html`)
with principle → contradiction → severity → possible remediation → status. Remediating any entry
is a future, separate Ines decision (via `/decizie`); do not "fix" register items opportunistically,
and do not reconcile foundation documents to shipped code — that exact mistake was made once and
reversed (see D-14).
...
> the UI). The luxe revamp is v133→v144; **the app has since moved on to v155** (see next box).
>
...
- **Staging preview (official process — skill `/staging`).** Cloudflare Pages previews every
  **non-production branch** at `<branch>.mi-dia-app.pages.dev`. A **permanent `staging` branch** gives a
  ...
  (that's a `/ship` step). First-time only: Cloudflare → Workers & Pages → mi-dia-app → Settings → Builds &
  deployments → Preview deployments → "All non-production branches". Full step list = the `/staging` skill.
- **⭐ Partial-promotion principle (single-file app — a rule every new agent must know).** Because the app
  is ONE monolithic HTML file (`index.html` = a copy of a whole `mi-dia-vNN.html`), **`git cherry-pick` does
  NOT isolate a feature** — cherry-picking a per-felie commit only adds a dormant `mi-dia-vNN.html` file, it
  never changes `index.html`, so the feature doesn't go live. **Partial promotion = choosing which build
  goes live:** you promote the `vNN` at which the wanted feature is complete AND nothing unwanted is present
  yet, making THAT `vNN` the new `index.html` (a `/ship` targeted at an intermediate `vNN`, not the newest).
  To KEEP partial promotion possible, **build this way:** (1) **order the work** so anything you might ship
  earlier is complete at an earlier `vNN`, and optional/risky/**additive** features come **LAST** (e.g. the
  „Floarea vie" arc put Garden/S4 last → the flower can promote at v182 while Garden waits at v183); (2) a
  genuinely independent feature → its **own branch off `main`** or an **additive module** (`ritual.js`/
  `cycle.js`/`onboard.js`/`garden` = separate file + a small mount) — the closest thing to an independently
  promotable unit; (3) every `vNN` must be a **coherent, shippable state** (validates + e2e green, no half
  features) or it isn't a valid cut-line; (4) when promoting an intermediate `vNN`, **reconcile e2e** — specs
  testing later features (e.g. `garden.spec.js` for S4) must not run against that build. Cherry-pick is only
  clean for **orthogonal** changes (a doc, a test, `sw.js`, an additive module file), never for feature code
  inside the monolith. See the `/staging` skill for the full rationale + steps.
...
- `[ ]` D14. Long-term: public version (lifetime pay-what-fair, NO subscription — Constitution D-13).
...
- `rituals` (v145→v155): array of `{ id, name, identity, cue:{type:"time"|"after", value}, twoMin, area,
  ...
  toggles TODAY's `keyFor` in/out of `log` (v154: always TODAY, not the viewed day; past-day backfill is
  explicit via the tappable Progress mini-calendar). Included in backup. Flag `rit_seeded_v1` = first-run
  seed marker (source of truth for "first run"; prevents re-seeding). Module = `ritual.js` (inlined).
...
> tracker, D14 public version (lifetime pay-what-fair, no subscription — Constitution D-13).
>
...
> UX-coherence E1–E5/E7 (**E6 date-nav REZOLVAT în v156**), B6 "min" clip, D14 public lifetime (fara
> abonament — Constitutia D-13; acum are plan de monetizare — vezi build-plan-ul). Possible future: extend `freq` beyond "daily", idei de retenție
...
- Reference modules: **`cycle.js`**, **`persist.js`**, **`ritual.js`** (v145+, Ritualuri) and **`onboard.js`**
  (v150+, guided onboarding) — each a clean source file at root mirroring the block INLINED into the app.
  `cycle.js`/`persist.js` are fully self-contained (portable `window.storage||localStorage`, own i18n/CSS).
